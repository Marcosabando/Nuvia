      ...
      <div className="max-w-7xl mx-auto space-y-8 p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-sm sm:text-base text-white mt-1">{recentItems.length} archivos accedidos recientemente</p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className="bg-gradient-to-br from-white to-nuvia-rose/10 border border-nuvia-rose/30 shadow-nuvia-soft rounded-2xl hover:shadow-nuvia-glow transition-all">
            <CardContent className="p-5">
              ...
              <p className="text-2xl font-bold mt-2 text-nuvia-deep truncate">{stats?.mostRecent?.name || "Sin datos"}</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-white to-nuvia-mauve/10 border border-nuvia-mauve/30 shadow-nuvia-soft rounded-2xl hover:shadow-nuvia-glow transition-all">
            <CardContent className="p-5">
              ...
              <p className="text-2xl font-bold mt-2 text-nuvia-deep">{stats?.counts.today || 0} archivos</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-white to-nuvia-deep/10 border border-nuvia-deep/30 shadow-nuvia-soft rounded-2xl hover:shadow-nuvia-glow transition-all">
            <CardContent className="p-5">
              ...
              <p className="text-2xl font-bold mt-2 text-nuvia-deep">{stats?.counts.week || 0} archivos</p>
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent className="p-6">
            ...
            <div className="space-y-4">
              {recentItems.map((item, index) => (
                <div key={`${item.type}-${item.id}`} className="flex gap-4 group hover:bg-gradient-to-r hover:from-nuvia-peach/10 hover:to-nuvia-rose/10 p-4 rounded-lg transition-all">
                  <div className="flex flex-col items-center">
                    <div className="w-3 h-3 rounded-full bg-gradient-to-r from-nuvia-rose to-nuvia-mauve ring-4 ring-nuvia-rose/20" />
                    {index < recentItems.length - 1 && <div className="w-0.5 h-20 bg-gradient-to-b from-nuvia-peach/50 to-nuvia-rose/30 mt-2" />}
                  </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <div>
                            <h3 className="font-medium text-nuvia-deep hover:text-nuvia-rose cursor-pointer transition-colors" onClick={() => handleOpen(item.name, item.path)}>
                              {item.title}
                            </h3>
                        ...
                        <div className="flex items-center gap-2 ml-13">
                          ...
                          <span className="text-xs text-nuvia-deep">Subido {getRelativeTime(item.uploadedAt)}</span>
                        </div>
                      </div>

                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button size="sm" variant="ghost" className="h-8 w-8 hover:bg-nuvia-peach/20 rounded-lg" onClick={() => window.open(getFileUrl(item.path), "_blank")}>
        ...
        {recentItems.length === 0 && (
          <Card className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-nuvia-soft border border-nuvia-peach/30">
            <CardContent className="py-12 text-center">
              ...
              <p className="text-sm text-nuvia-mauve/70 mt-2">Los archivos que subas aparecerán aquí para un acceso rápido</p>
            </CardContent>
          </Card>
        )}
      </div>
