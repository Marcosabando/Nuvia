...
export const VideoGallery = ({ viewMode = "grid" }: VideoGalleryProps) => {
  ...
  const getThumbnailUrl = (video: Video) => {
    if (video.thumbnailPath) return buildUploadsUrl(video.thumbnailPath);
  ...
  const getVideoUrl = (video: Video) => {
    if (video.videoPath) return buildUploadsUrl(video.videoPath);
    if (video.userId && video.filename) return buildUploadsUrl(`uploads/${video.userId}/videos/${video.filename}`);
    return "";
  };
                    ...
                    <DropdownMenuPortal>
                      <DropdownMenuSubContent className="w-48">
                        {foldersLoading ? (
                          ...
                          folders
                            .map((folder) => {
                              const actualFolderId = Number(folder.folderId ?? folder.id);
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div className="flex items-center gap-4 flex-1 min-w-0">
          <div className="relative flex-1 max-w-md">
            <Search className="absolute left-3 top-1/2 transform -translate-y-0.5 text-nuvia-deep/40 w-4 h-4" />
            <Input
      ...
      {loading && videos.length === 0 ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-nuvia-deep/60 mb-4">
            {videos.length === 0 ? "Comienza subiendo tu primer video" : "Intenta con otros términos de búsqueda"}
          </p>
        </div>
      ) : (
        <>
          ...
          {totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between pt-6 border-t border-nuvia-silver/30 gap-4">
              <div className="text-sm text-nuvia-deep/60 text-center sm:text-left">
                Mostrando {startIndex + 1}-{Math.min(startIndex + itemsPerPage, filteredVideos.length)} de{" "}
