...
const getImageUrl = (image: any, useThumbnail = false): string => {
  const pathKey = useThumbnail ? image.thumbnailPath || image.mediumPath : image.mediumPath || image.imagePath;
  ...
  return (
    <>
      <div className="space-y-6">
        ...
        {loading && filteredImages.length === 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-nuvia-deep mb-2">No hay imágenes</h3>
            <p className="text-nuvia-deep/60 mb-4">Comienza subiendo tu primera imagen</p>
          </div>
        ) : (
          <>
            <div
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-2 gap-2">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                >
                                  <Heart
                                    className={`w-3 h-3 ${
                                      image.isFavorite ? "text-red-500 fill-current" : "text-gray-600"
                                    }`}
                                  />
                                </Button>
                                ...
                                <DropdownMenu>
                                  ...
                                  <DropdownMenuContent align="end" className="w-48 z-[9999]">
                                    <DropdownMenuItem onClick={() => toggleFavorite(image.id)}>
                                      <Heart
                                        className={`w-4 h-4 mr-2 ${image.isFavorite ? "text-red-500 fill-current" : ""}`}
                                      />
                ...
                return (
                  ...
                  >
                    <CardContent className="p-0 relative">
                      <div className="aspect-square bg-gray-50 relative overflow-hidden cursor-pointer" onClick={() => setSelectedImage(image)}>
                        <img
                          src={getImageUrl(image, true)}
                          alt={displayName}
            ...
            {totalPages > 1 && (
              <div className="flex flex-col sm:flex-row items-center justify-between pt-6 border-t border-nuvia-silver/30 gap-4">
                <div className="text-sm text-nuvia-deep/60 text-center sm:text-left">
                  Mostrando {startIndex + 1}-{Math.min(startIndex + itemsPerPage, filteredImages.length)} de{" "}
            ...
            <div className="flex flex-col md:flex-row min-h-full">
              <div className="flex-1 flex items-center justify-center p-4 min-h-[40vh] md:min-h-[60vh]">
                <img
                  ...
                  onError={(e) => {
                    e.currentTarget.src = getImageUrl(selectedImage, true);
                  }}
                />
              </div>
                  ...
                  <div className="min-w-0 pr-2">
                    ...
                    {selectedImage.title && selectedImage.title !== selectedImage.originalFilename && (
                      <p className="text-sm text-nuvia-deep/60 mt-1 break-words">
                        Original: {selectedImage.originalFilename}
                      </p>
                    )}
                  </div>
                </div>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setRenameModal({
                          open: true,
                          image: selectedImage,
                          name: selectedImage.title || selectedImage.originalFilename,
                        });
                      }}
                    >
                      <Edit3 className="w-4 h-4 mr-2" />
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setRenameModal({ open: false, image: null, name: "" })}
      >
        <DialogContent className="sm:max-w-md">
              ...
              <div className="flex items-center gap-3 p-3 bg-nuvia-silver/10 rounded-lg border border-nuvia-silver/30">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate text-nuvia-deep">
                    {renameModal.image.title || renameModal.image.originalFilename}
                  </p>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setRenameModal({ open: false, image: null, name: "" })}
              disabled={isRenaming}
              className="border-nuvia-silver/30"
            >
              Cancelar
      ...
      <Dialog open={deleteModal.open} onOpenChange={(open) => !open && setDeleteModal({ open: false, image: null })}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>¿Estás seguro de que quieres mover esta imagen a la papelera?</DialogDescription>
          </DialogHeader>
          {deleteModal.image && (
            <div className="space-y-4 py-4">
